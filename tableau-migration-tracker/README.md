...
This folder contains the migration control plane shown in the deck. The tracker
compares the known Redshift/dbt object inventory against the target Iceberg
catalog so cutover status can be reviewed by schema, folder, owner, and
required-for-cutover flag.
...
- [migration_tracker.sql](migration_tracker.sql) - dbt model that classifies
  source objects as migrated, required but not migrated, or optional.
